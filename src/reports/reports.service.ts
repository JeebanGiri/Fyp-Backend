...
export class ReportsService {
  ...
  async updateReport(id: string, updateReportDto: UpdateReportDto, user: User) {
    ...
    if (user.role === UserRole.hotel_admin || user.role === UserRole.super_admin) {
      where['id'] = id;
    } else {
      where['id'] = id;
    ...
    const report = await this.dataSource.getRepository(Report).findOne({ where });

    if (!report) {
      throw new NotFoundException('Report not found');
    }

    return await this.dataSource.getRepository(Report).save({ ...report, ...updateReportDto });
  }
